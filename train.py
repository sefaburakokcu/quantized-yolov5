...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    binary = False
    if "bit_width" in model_definition.keys():
        if model_definition["bit_width"][0][1] == 1:
            binary = True
    ...
    train_loader, dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                              ...
                                              prefix=colorstr('train: '), binary=binary)
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                       ...
                                       prefix=colorstr('val: '), binary=binary)[0]
    ...
    model_definition['anchors'] = (model.module.model[-1].anchors*model.module.model[-1].stride).cpu().tolist() if hasattr(model, 'module') else (model.model[-1].anchors*model.model[-1].stride).cpu().tolist()  
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if binary:
                imgs = 2.0 * imgs - torch.tensor([1.0], device=device)
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
